...
import { FileText, Plus, Search, Trash2, Calendar, Loader2 } from "lucide-react";
...
export default function NotasPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [notesResponse, clientsResponse, treatmentsResponse] = await Promise.all([
          fetch('/api/notas'),
          fetch('/api/pacientes'),
          fetch('/api/tratamientos')
        ]);

        if (!notesResponse.ok || !clientsResponse.ok || !treatmentsResponse.ok) {
    ...
    const filtered = notes.filter(
      (note) =>
        ...
        note.client && note.treatment && (
          ...
          note.treatment.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const addNote = async () => {
    ...
    try {
      ...
      setNewNote({ client: "", treatment: "", description: "", createdBy: "Admin" });
      setError(null);
    } catch (error) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="notes-form">
          ...
          <CardContent className="space-y-4">
            ...
            <Button
              ...
              disabled={!newNote.client || !newNote.treatment || !newNote.description}
            >
        ...
        <Card className="notes-list">
          <CardHeader>
            ...
            <CardDescription>Últimas notas clínicas del sistema</CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      <h4 className="font-medium text-primary">
                        {note.client?.name || 'N/A'} {note.client?.lastName || ''}
                      </h4>
                    ...
                    <div className="flex items-center space-x-2">
                      <Badge className={getCategoryColor(note.treatment?.category || 'General')}>
