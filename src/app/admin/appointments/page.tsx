...
export default function TurnosPage() {
    ...
    const fetchAppointments = async () => {
      try {
        ...
        const events: CalendarEvent[] = data
          ...
          ) // Filtrar turnos con datos válidos
  ...
  useEffect(() => {
    const filterAppointments = () => {
        ...
        case "week":
          ...
          filtered = appointments.filter((appointment) => {
            ...
            return (
              ...
              appointmentDate.getTime() <= endOfWeek.getTime()
            );
          });
          break;
        case "month":
      ...
      const validFiltered = filtered.filter(
        (appointment) => appointment.client && appointment.treatment,
      );
      validFiltered.sort(
  ...
  const handleCreateAppointment = async () => {
    ...
    try {
      ...
      const endDate = addMinutes(
        startDate,
        createdAppointment.treatment.duration,
      );
  ...
  const handleUpdateAppointment = async () => {
    if (
      !editAppointment.client ||
      !editAppointment.treatment ||
    ...
    try {
      ...
      const endDate = addMinutes(
        startDate,
        updatedAppointment.treatment.duration,
      );
        ...
        <div className="turnos-header flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-primary/70 mt-1">
              Calendario de citas y horarios
            </p>
          </div>
          <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                ...
                <DialogDescription>
                  Selecciona un paciente, tratamiento y programa la fecha y hora
                  del turno.
                </DialogDescription>
              </DialogHeader>
        ...
        <div className="turnos-header grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card className="bg-gradient-to-br from-primary to-primary/80 text-white">
            ...
            <CardContent>
              ...
              <p className="text-xs text-white/80">
                {
                  todayAppointments.filter((app) => app.status === "Scheduled")
                    .length
                }{" "}
                programados
              </p>
          ...
          <Card className="bg-gradient-to-br from-sky-500 to-sky-600 text-white">
            ...
            <CardContent>
              ...
              <p className="text-xs text-white/80">
                {nextAppointment
                  ? `${nextAppointment.client.name} - ${nextAppointment.treatment.name}`
                  : "Sin turnos programados"}
              </p>
            </CardContent>
          </Card>
