...
export default function PatientDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchPatientData = async () => {
      try {
        ...
        const [patientResponse, appointmentsResponse, notesResponse, treatmentsResponse] = await Promise.all([
        ...
        const [patientData, appointmentsData, notesData, treatmentsData] = await Promise.all([
        ...
        const patientAppointments = appointmentsData.filter((appointment: any) => 
          appointment.client && appointment.client._id === id
        );
        const patientNotes = notesData.filter((note: any) => 
          note.client && note.client._id === id
        );
  ...
  const getNextAppointment = () => {
    ...
    const futureAppointments = appointments
      .filter(appointment => appointment.client && appointment.treatment && new Date(appointment.date) > today)
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        ...
        <span className="ml-2 text-primary">Cargando datos del paciente...</span>
      </div>
    );
  }
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            <div className="space-y-3 sm:space-y-4">
              <div>
                <Label className="text-xs sm:text-sm font-medium text-primary">Nombre</Label>
                <p className="text-base sm:text-lg break-words">{patient.name}</p>
              </div>
              <div>
                <Label className="text-xs sm:text-sm font-medium text-primary">Apellido</Label>
                <p className="text-base sm:text-lg break-words">{patient.lastName}</p>
              </div>
              <div>
                <Label className="text-xs sm:text-sm font-medium text-primary">DNI</Label>
                <p className="text-base sm:text-lg">{patient.dni}</p>
              </div>
              <div>
                <Label className="text-xs sm:text-sm font-medium text-primary">Teléfono</Label>
            ...
            <div className="space-y-3 sm:space-y-4">
              ...
              <div>
                <Label className="text-xs sm:text-sm font-medium text-primary">Fecha de Nacimiento</Label>
                <p className="text-base sm:text-lg">
                  {format(parseISO(patient.birthDate), 'dd/MM/yyyy', { locale: es })}
                </p>
              </div>
              <div>
                <Label className="text-xs sm:text-sm font-medium text-primary">Edad</Label>
                <p className="text-base sm:text-lg">{calculateAge(patient.birthDate)} años</p>
              </div>
              {patient.obraSocial && (
                <div>
                  <Label className="text-xs sm:text-sm font-medium text-primary">Obra Social</Label>
                  <p className="text-base sm:text-lg break-words">{patient.obraSocial}</p>
                </div>
              )}
              {patient.address?.street && (
                <div>
                  <Label className="text-xs sm:text-sm font-medium text-primary">Dirección</Label>
                  <p className="flex items-center text-base sm:text-lg">
                    <MapPin className="w-3 h-3 sm:w-4 sm:h-4 mr-2 text-red-500 flex-shrink-0" />
                    <span className="break-words">{patient.address.street}</span>
                  </p>
                </div>
              )}
            </div>
          ...
          <CardContent>
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 p-3 sm:p-4 bg-white rounded-lg border border-blue-200">
              <div className="flex-1">
                <p className="font-medium text-base sm:text-lg break-words">{nextAppointment.treatment?.name || 'Tratamiento no disponible'}</p>
                <p className="text-xs sm:text-sm text-muted-foreground break-words">{nextAppointment.treatment?.description || ''}</p>
              </div>
              <div className="sm:text-right border-t sm:border-t-0 pt-2 sm:pt-0 sm:border-l sm:pl-4">
                <p className="text-base sm:text-lg font-medium text-primary">
                  {format(parseISO(nextAppointment.date), 'dd/MM/yyyy', { locale: es })}
                </p>
                <p className="text-xs sm:text-sm text-muted-foreground">
                  {format(parseISO(nextAppointment.date), 'HH:mm', { locale: es })}
                </p>
              </div>
            </div>
          </CardContent>
            ...
            <div className="space-y-3 sm:space-y-4">
              ...
              <div className="p-3 sm:p-4 border rounded-lg bg-gray-50">
                <div className="space-y-3">
                  <div>
                    <Label htmlFor="treatment" className="text-xs sm:text-sm">Tratamiento</Label>
                    <select
                      id="treatment"
                  ...
                  <Button onClick={addNote} className="w-full text-sm sm:text-base">
              ...
              <div className="space-y-2 sm:space-y-3">
                {notes
                  ...
                  .map((note) => (
                  <div key={note._id} className="p-2 sm:p-3 border rounded-lg bg-white">
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 mb-2">
                      <div className="flex-1">
                        <p className="font-medium text-sm sm:text-base break-words">{note.treatment?.name || 'Tratamiento no disponible'}</p>
                      </div>
                      <div className="flex items-center justify-between sm:justify-end gap-2">
                        <p className="text-xs sm:text-sm text-muted-foreground whitespace-nowrap">
                          {format(parseISO(note.createdAt), 'dd/MM/yyyy', { locale: es })}
                        </p>
                    ...
                    <p className="text-xs sm:text-sm break-words">{note.description}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-2 sm:space-y-3">
              {appointments.length === 0 ? (
                ...
                appointments
                  ...
                  .map((appointment) => (
                    <div key={appointment._id} className="p-2 sm:p-3 border rounded-lg bg-white">
                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 mb-2">
                        <div className="flex-1">
                          <p className="font-medium text-sm sm:text-base break-words">{appointment.treatment?.name || 'Tratamiento no disponible'}</p>
                        </div>
      ...
      {isEditNoteModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-4 sm:p-6 w-full max-w-md">
            <h3 className="text-base sm:text-lg font-semibold mb-3 sm:mb-4">Editar Nota</h3>
            <div className="space-y-3 sm:space-y-4">
              <div>
                <Label htmlFor="edit-treatment" className="text-xs sm:text-sm">Tratamiento</Label>
                <select
                  id="edit-treatment"
                  value={editNoteData.treatment}
                  onChange={(e) => setEditNoteData({...editNoteData, treatment: e.target.value})}
                  className="w-full p-2 text-sm sm:text-base border rounded-md"
                >
                  <option value="">Seleccionar tratamiento</option>
                  {treatments.map((treatment) => (
              ...
              <div>
                <Label htmlFor="edit-description" className="text-xs sm:text-sm">Descripción</Label>
                <Textarea
                  id="edit-description"
